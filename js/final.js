...
function inicializarAvaliacao() {
    ...
    fetch("json/final.json")
        ...
        .then(data => {
            ...
            let dezQuestoes = questoesEmbaralhadas.slice(0, 5);
            ...
            dezQuestoes.forEach((item, index) => {
        ...
        .catch(error => console.error("Erro ao carregar a avaliação final:", error));
}

function validarRespostaFinal(questao, alternativaClicada, container, questaoId) {
...
function exibirEncerramentoCurso() {
    ...
    if (pontuacaoTotal >= 80) {
        ...
        resultadoBox.querySelector("#btn-certificado").addEventListener("click", () => {
            alert("Parabéns pela formatura! Seu certificado está sendo gerado...");
        });
        ...
        resultadoBox.querySelector("#btn-recomecar").addEventListener("click", () => {
            inicializarAvaliacao();
